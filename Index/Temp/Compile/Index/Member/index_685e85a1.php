...
URL = 'http://localhost/wenda/index.php/Index/Member/index/uid/13';
...
ACTION = 'http://localhost/wenda/index.php/Index/Member/index';
...
HISTORY = 'http://localhost/wenda/index.php';
</script>
...
<ul>
        ...
        <li class='mygold     <?php if($flag=='my_gold' ){ ?>cur<?php } ?>'>
            <a href="<?php echo U('Member/my_point',array('uid'=>$_GET['uid']));?>">我的金币</a>
        </li>



        <?php if($hd['session']['uid']==$hd['get']['uid']){ ?>
...
<!-- 左侧结束 -->
		<div id='right'>
			...
			<div class='list'>
				<p><span><?php echo $rank;?>的提问 <b>(共<?php echo $member['ask'];?>条)</b></span><a href="<?php echo U('Member/my_ask',array('uid'=>$_GET['uid']));?>">更多>></a></p>
				<table>
        ...
        if (empty($ask)) {
			...
			<div class='list'>
				<p><span><?php echo $rank;?>的回答 <b>(共20条)</b></span><a href="">更多>></a></p>
				<table>
                         <?php if(empty($answerInfo)){ ?>
					<tr height='140'>
						<td>你还没有进行过回答</td>
					</tr>
        ...
        if (empty($answerInfo)) {
            ...
            foreach ($answerInfo as $d) {
                ...
                $hd['list'][d]['first']=($listId == 0);
                //最后一个值
                $hd['list'][d]['last']= (count($answerInfo)-1 <= $listId);
                //总数
                $hd['list'][d]['total']++;
                //增加数
                $listId++;
                ...
                ?>
					<tr>
						<td class='t1'>
							<a href=""><?php echo $d['content'];?></a><span>[<?php echo $d['title'];?>]</span>
						</td>
						<td><?php echo $d['answer'];?></td>
